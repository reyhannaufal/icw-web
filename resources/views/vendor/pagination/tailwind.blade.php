            ...
            <div class="flex h-8 font-medium items-center">
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_string($element))
                        <div class="w-8 md:flex justify-center items-center hidden  cursor-pointer leading-5
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            @if ($page == $paginator->currentPage())
                                <div class="w-8 mx-2 md:flex justify-center items-center hidden  cursor-pointer leading-5
                            ...
                            @else
                                <div class="md:flex justify-center items-center hidden  cursor-pointer leading-5
                                                transition duration-150 ease-in border-0 w-8 mx-2">
                                    <a href="{{ $url }}">{{ $page }}</a>
                                </div>
                            @endif
                        @endforeach
